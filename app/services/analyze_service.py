...
class AnalyzeService:
    ...
    def check_autocorrelation(self):
        ...
        for col in self._numeric_columns:
            residuals = (
                self._df[col] - self._df[col].mean()
            )  # разность между соседними значениями
            dw_statistic = durbin_watson(residuals)
            self.__autocorr_info[col] = (
                True if dw_statistic < 1.5 or dw_statistic > 2.5 else False
            )
    ...
    def check_seasonal_trend(self):
        ...
        for col in self._numeric_columns:
            ...
            self.__trend_info["seasonal"][col] = (
                True if result.resid.isnull().any() else False
            )
    ...
    def analyze(self):
        ...
        try:
            ...
            self._errors["trends_check"] = (
                {}
                if "trends_check" not in self._errors.keys()
                else self._errors["trends_check"]
            )
