...
def get_calculation_history():
    ...
    response = app.response_class(
        response=flask_json.dumps(result), mimetype="application/json"
    )
    return response, 200


@bp.patch("/change_username")
@swag_from(swagger_personal.CHANGE_USERNAME)
@login_required
def change_username():
    update_info = UpdateUserSchema(
        id=current_user.get_id(), username=request.values["new_username"]
    )
    try:
